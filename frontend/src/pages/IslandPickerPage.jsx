        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, width: '100%', maxWidth: 320 }}>
          {Object.values(ISLANDS).map(island => (
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--ocean)'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(45,212,191,0.2)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none'; }}
            >
              <span style={{ fontSize: '2.2rem', lineHeight: 1 }}>🌊</span>
              <span style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--dark)' }}>{island.name}</span>
            </button>
          ))}
        </div>
